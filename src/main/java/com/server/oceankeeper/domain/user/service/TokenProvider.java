...
public class TokenProvider {
    ...
    private static final long ACCESS_TOKEN_EXPIRE_TIME = 1000 * 60 * 60 * 24 * 7;   // 1 week
    ...
    public boolean verifyToken(String token) {
        try {
            ...
            log.debug("잘못된 JWT 서명입니다. token :{}", token);
        } catch (ExpiredJwtException e) {
            log.debug("만료된 JWT 토큰입니다. token :{}", token);
            throw new ExpiredTokenException("만료된 JWT 토큰입니다");
        } catch (UnsupportedJwtException e) {
            log.debug("지원되지 않는 JWT 토큰입니다. token :{}", token);
        } catch (IllegalArgumentException e) {
            log.debug("JWT 토큰이 잘못되었습니다. token :{}", token);
        }
        return false;
    }
